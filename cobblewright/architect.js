...
async function getArchitectAdvice(username, trigger = 'auto') {
  ...
  if (sharedState.getActiveProject) {
    try {
      const activeProject = await sharedState.getActiveProject(username);
      if (activeProject) {
      ...
      console.warn('[Brain] Failed to load active project context:', error.message);
    }
  }

  if (sharedState.findRelevantMemories) {
    try {
      const memoryQuery = `build advice ${terrain.biome} ${playerStates.get(username)?.currentStyle || 'any'}`;
      const relevantMemories = await sharedState.findRelevantMemories({ username, query: memoryQuery, limit: 3 });
      if (Array.isArray(relevantMemories) && relevantMemories.length > 0) {
        const memorySummary = relevantMemories
          .map((entry) => {
            const short = String(entry.content || '').replace(/\s+/g, ' ').trim().slice(0, 140);
            return `- [${entry.type || 'memory'}] ${short}`;
          })
          .join('\n');
        contextSections.push(`Relevant Past Context:\n${memorySummary}`);
      }
    } catch (error) {
      console.warn('[Brain] Failed to load relevant memories:', error.message);
    }
  }

  if (terrain.buildProfile) {
    const profile = terrain.buildProfile;
    const utilityList = profile.utilityBlocks.length > 0 ? profile.utilityBlocks.join(', ') : 'none detected';
    contextSections.push(
      `Build Site Assessment:\n` +
      `- Site readiness: ${profile.siteReadiness}.\n` +
      `- Terrain: ${profile.terrainFlatness}.\n` +
      `- Shelter state: ${profile.shelterState}.\n` +
      `- Lighting: ${profile.lightingState}.\n` +
      `- Nearby man-made signal: ${profile.manMadeRatio}% (higher means more existing structure).\n` +
      `- Nearby utility blocks: ${utilityList}.`
    );
  }
...
- If utility blocks already exist, propose upgrades/optimization instead of repeating beginner setup.
...
Now, generate your JSON response.`;
  try {
    ...
    if (sharedState.updateProjectFromAdvice) {
      sharedState.updateProjectFromAdvice(username, adviceJson).catch((error) => {
        console.warn('[Project] Failed to update project from advice:', error.message);
      });
    }
